 ...
 * Pipeline: passphrase → BLAKE2s-256 → private_key → G·k → uncompressed_pubkey[64B]
 *           → Keccak-256 → last 20 bytes → Ethereum address (0x + hex)
 */
...
DECLSPEC void keccak_256_64 (PRIVATE_AS const u32 *in, PRIVATE_AS u32 *out)
{
  ...
  // 64 bytes < 136 byte rate, so single block (in big-endian word order)
...
KERNEL_FQ KERNEL_FA void m35911_mxx (KERN_ATTR_RULES ())
{
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos++)
  {
    ...
    // Step 1: BLAKE2s-256 of passphrase → private key

    blake2s_ctx_t b2s_ctx;

    blake2s_init   (&b2s_ctx);
    blake2s_update (&b2s_ctx, p.i, p.pw_len);
    blake2s_final  (&b2s_ctx);

    // BLAKE2s output is in little-endian u32 words
    u32 prv_key[9];

    prv_key[0] = hc_swap32_S (b2s_ctx.h[7]);
    prv_key[1] = hc_swap32_S (b2s_ctx.h[6]);
    prv_key[2] = hc_swap32_S (b2s_ctx.h[5]);
    prv_key[3] = hc_swap32_S (b2s_ctx.h[4]);
    prv_key[4] = hc_swap32_S (b2s_ctx.h[3]);
    prv_key[5] = hc_swap32_S (b2s_ctx.h[2]);
    prv_key[6] = hc_swap32_S (b2s_ctx.h[1]);
    prv_key[7] = hc_swap32_S (b2s_ctx.h[0]);
    prv_key[8] = 0;

    // Step 2: EC point multiplication
    ...
    // Step 3: Ethereum uses uncompressed public key (x || y) for Keccak-256
    // Store as big-endian byte words
    ...
    // Step 4: Keccak-256 of uncompressed public key, take last 20 bytes
